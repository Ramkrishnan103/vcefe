        ...
        <>
            {
                ...
                link ? <div className='d-flex mt-5 justify-content-end'>
                    {onEditRecord ?
                        <button onClick={( event ) => onSubmit( event )} className='btn btn-primary me-3 save-btn' type='submit'
                            disabled={editDisabled || isSaving} ref={ref}>
                            {isSaving ? placeholderText( "globally-saving-btn-label" ) : placeholderText( "globally.update-btn" )}
                        </button>
                        :
                        <button onClick={( event ) => onSubmit( event )} className='btn btn-primary me-3 save-btn' type='submit'
                    ...
                    <Modal.Footer children='justify-content-start' className='pt-0'>
                        {onEditRecord ?
                            <button onClick={( event ) => onSubmit( event )} className='btn btn-primary me-2' type='submit'
                                ...
                                {placeholderText( "globally-update-btn-label" )}
                            </button> :
                            <button onClick={( event ) => onSubmit( event )} className='btn btn-primary me-2' type='submit'
