...
const SalaryPreparation = (props) => {
  const { show, handleClose, title, fetchSalaryDetailsFilter, salaryDetail } =
    props;
  ...
  const loadValues = (filter) => {
    ...
    fetchSalaryDetailsFilter(values, filter, true, "save");
  };
  ...
  const submitClick = () => {
    ...
    if (
      salaryDetail?.data?.some(
        (item) =>
          item?.attributes?.salaryYear === preparationValue.year &&
          item?.attributes?.salaryMonth === preparationValue.month
      )
    ) {
      dispatch(
        addToast({
        ...
        <Modal.Body>
          <div className="row">
            <div className="col-md-2 mb-3">
              <h3 className="mt-3">{getFormattedMessage("Year.title")}</h3>
            </div>
            ...
            <div className="col-md-2 mb-3">
              <h3 className="mt-3">{getFormattedMessage("month.title")}</h3>
            </div>
            <div className="col-md-4">
              <ReactSelect
                data={monthOptions}
          ...
          <div className="row">
            ...
            <div className="col-md-8">
              <ReactSelect
                ...
                value={options.find(
                  (option) => option.label === preparationValue.groupBy
                )}
                data={options}
