...
import { fetchPayrollReportDetailsFilter } from '../../../store/action/payrollReportDetailsAction';

const PayrollDetailed = (props) => {
  const { companyConfig, fetchCompanyConfig,fetchPayrollReportDetailsFilter ,payrollReportDetails} = props;
  ...
  const loadValues = (filter) => {
    ...
    let values = `?year=${year ? year.value : ""}&month=${
      month ? month.value : ""
    }`;
    console.log(values);

    fetchPayrollReportDetailsFilter(values, filter, true);
  };
  ...
  useEffect(() => { 
    fetchCompanyConfig();
    // fetchPayrollReportDetailsFilter();
  }, [fetchCompanyConfig]);
  ...
  const itemsValue =
  payrollReportDetails?.length > 0 &&
  payrollReportDetails.map((payroll) => {
    return {
      empId: payroll?.empId,
      empName: payroll?.empName,
      basicPay:payroll?.basicPay,
      hra:payroll?.hra,
      conveyance:payroll?.conveyaences,
      others:payroll?.others,
      esi:payroll?.employeeEsi,
      pf:payroll?.employeePf,
      lwf:payroll?.lwf,
      taxDeduc:payroll?.taxDeductions,
      lop:payroll?.lop,
...
export default connect(mapStateToProps, { fetchCompanyConfig ,fetchPayrollReportDetailsFilter})(PayrollDetailed);
